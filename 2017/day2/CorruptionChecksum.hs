...
diceData :: String -> [[Int]]
diceData xs = ((fmap . fmap) read . fmap words . lines) xs
...
divideElements :: (Int, Int) -> Int
divideElements (0, 0) = 0
divideElements x = div (fst x) (snd x)

corruptionChecksum :: String -> Int
corruptionChecksum x = sum . fmap distanceBetweenMinMax . diceData $ x

corruptionChecksum' :: String -> Int
corruptionChecksum' x = 
  sum . fmap (divideElements . findDivisors . reverse . sort) . diceData $ x

input :: String
input = [r|104	240	147	246	123	175	372	71	116	230	260	118	202	270	277	292
